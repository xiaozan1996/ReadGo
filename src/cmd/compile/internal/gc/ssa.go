...
func buildssa(fn *Node, worker int) *ssa.Func {
	...
	/* 	开放编码作为一种优化 defer 关键字的方法，它不是在所有的场景下都会开启的，开发编码只会在满足以下的条件时启用：
...
// 传入 defer 关键字的函数和参数都会存储在如下所示的 cmd/compile/internal/gc.openDeferInfo 结构体中
...
/* 负责了为所有函数和方法调用生成中间代码，它的工作包括以下内容：
...
func (s *state) call(n *Node, k callKind, returnResultAddr bool) *ssa.Value {
	...
	var call *ssa.Value
	//1.14 defer存在第三方式作为defer简单时候的优化 为开放编码
	/* 	然而开放编码作为一种优化 defer 关键字的方法，它不是在所有的场景下都会开启的，开发编码只会在满足以下的条件时启用：

	函数的 defer 数量少于或者等于 8 个；
	函数的 defer 关键字不能在循环中执行；
	函数的 return 语句与 defer 语句的乘积小于或者等于 15 个 */
	if k == callDeferStack { // 在栈上初始化 defer 结构体
		...
		switch {
		case k == callDefer:
			call = s.newValue1A(ssa.OpStaticCall, types.TypeMem, ssa.StaticAuxCall(deferproc, ACArgs, ACResults), s.mem()) //当 runtime._defer 分配在堆上时，Go 语言的编译器不仅将 defer 转换成了 runtime.deferproc，还在所有调用 defer 的函数结尾插入了 runtime.deferreturn
		case k == callGo:
